...
it runs as a script: its `if __name__ == "__main__"` demo (the toy
ViT-projector-LLM stack, the CMER computation, the DeepStack feature merge)
executes there, and its stdout is relayed back here line by line.

The lesson demonstrates the VLM pattern on random tensors with no CUDA branch,
so this run proves the shipping path rather than any speed-up -- it costs about
a cent. The real card is needed by the other half of the pair,
`qwen3vl_cmer.py`, which runs the actual 8B model the lesson names and feeds
its CMER function real answers instead of random vectors.
...
`krauncher/` so they pick up that one .env:
    ...
    python ../phases/04-computer-vision/25-vision-language-models/code/main_remote.py
"""
